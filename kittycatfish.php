...
// create trigger shortcode
function kittycatfish_shortcode(){
	add_shortcode('kittycatfish_trigger', 'kittycatfish_print_trigger');
}

function kittycatfish_print_trigger(){
	$output = '<a name="kittycatfish_trigger" id="kittycatfish_trigger"></a>';
...
function kittycatfish_include_ad(){
	...
	if ($kc_ad !== false && is_object($kc_ad)){
		if (kittycatfish_check_cookies($kc_ad_postmeta->post_id) !== false){
			...
			// filter the content
			$ad_content = apply_filters('the_content', $kc_ad->post_content);
...
// shortcode creation
add_action('init', 'kittycatfish_shortcode');
...
if (!is_admin()){
	add_action('template_redirect', 'kittycatfish_test_post');
}
